...
class G2pTrainer():
    def __init__(self,
        ...
        # Create optimizer from config or use default Adam
        if optimizer_cfg is not None:
            self.optimizer = hydra.utils.instantiate(optimizer_cfg, params=model.parameters())
        else:
            from torch.optim import Adam
            self.optimizer = Adam(model.parameters(), lr=0.005, betas=(0.9, 0.98), eps=1e-9)
        print(f"Using optimizer: {self.optimizer.__class__.__name__}")
        
        # Create lr_scheduler from config or use default ExponentialLR
        if lr_scheduler_cfg is not None:
            self.scheduler = hydra.utils.instantiate(lr_scheduler_cfg, self.optimizer)
        else:
            from torch.optim.lr_scheduler import ExponentialLR
            self.scheduler = ExponentialLR(self.optimizer, gamma=0.8, verbose=True)
        print(f"Using learning rate scheduler: {self.scheduler.__class__.__name__}")
